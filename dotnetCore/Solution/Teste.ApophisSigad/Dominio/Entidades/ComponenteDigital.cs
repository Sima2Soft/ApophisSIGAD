...
{
    /// <summary>
    /// Entidade: ComponenteDigital
    /// Representa no Mundo Real: É uma cadeia de "bits" (objeto original) distinta que, sozinha ou combinada
    /// com outras cadeias de "bits", compõe um Documento Arquivístico <see cref="DocumentoArquivistico"/> digital.
    /// Cada Documento Arquivístico <see cref="DocumentoArquivistico"/> digital é composto de, pelo menos,
    /// um Componente Digital; 
    /// Alguns são compostos por vários Componentes Digitais.
    /// </summary>
    public class ComponenteDigital
    {
        ...
        [FichaIndividual(
            ...
            "Estabelecer a relação entre o documento e os componentes digitais necessários " +
            "para apresentá - lo.",
            ValoresDeObrigatoriedade.NaoSeAplica, ValoresDeObrigatoriedade.NaoSeAplica, ValoresDeObrigatoriedade.Obrigatorio,
            "Um documento pode ser formado por um ou mais componentes digitais, que " +
            " são os componentes físicos do documento.De modo geral, pode se dizer que os " +
            " componentes digitais são os arquivos de computador que formam um documento. " +
            "Cada    componente  deve    ser identificado    individualmente a   fim de  que o   documento " +
            " possa ser recuperado de maneira completa.",
            "Um relatório de atividades pode ser composto por diversas seções, sendo que " +
            " cada uma delas constitui um arquivo separado.Entretanto, todos esses arquivos " +
            " integram - se para a representação completa do documento. " +
            " A mesma situação aplica - se a documentos estruturados em bases de dados ou, " +
            " ainda, a documentos multimídia",
            "3.1.7 / 3.1.9 / 3.1.21"
            )]
        public virtual Nullable<long> ComponenteDigitalId { get; set; }

        public virtual IList<DocumentoArquivistico> DocumentosArquivisticos { get; private set; }
