...
namespace Teste.ApophisSigad.Dominio.Entidades
{
    /// <summary>
    /// Entidade Classe
    /// Representa no Mundo Real: Classe
    /// 
    /// Um Plano de Classificação <see cref="PlanoDeClassificacao" /> é composto por uma hierarquia de Classes,
    /// SubClasses<see cref="subclasses"/>, Grupos <see cref="Grupo"/>, SubGrupos <see cref="subgrupo"/> e tipos
    /// de Documentos Arquivísticos <see cref="DocumentoArquivistico"/> em uma estrutura
    /// de árvore, que podem ser identificados por códigos. No contexto do e-ARQ Brasil, a entidade Classe 
    /// <see cref="Classe"/> refere-se, de forma genérica, aos níveis citados. Assim, quando um requisito trata de
    /// uma classe, estão sendo considerados todos os níveis do Plano de Classificação <see cref="PlanoDeClassificacao"/>
    /// Cada classe pode conter:
    ///  - Outras Classes a ela subordinadas; ou
    ///  - Unidades de Arquivamento <see cref="UnidadeDeArquivamento"/>; e/ou
    ///  - Documentos Arquivísticos <see cref="DocumentoArquivistico"/>. 
    ///  Não são permitidas combinações entre a primeira e as demais opções. Uma classe que tenha outras classes a ela
    ///  subordinadas não pode conter Unidade de Arquivamento <see cref="UnidadeDeArquivamento"/>
    ///  ou Documentos Arquivísticos <see cref="DocumentoArquivistico"/>, mas 
    ///  uma classe que não será composta de outras classes a ela subordinadas pode ter Unidade de Arquivamento 
    ///  <see cref="UnidadeDeArquivamento"/> e/ou Documento Arquivístico <see cref="DocumentoArquivistico"/>.
    ///  Cada classe deve estar relacionada a um Tipo de Classe <see cref="TipoDeClasse"/>.
    /// </summary>
    public class Classe
    {
        #region Atributos
        ...
        public IList<Classe> SubClasses { get; private set; }
        public IList<Grupo> Grupos { get; private set; }
        public TipoDeClasse Tipo { get; private set; }
        public IList<DocumentoArquivistico> DocumentosArquivisticos { get; private set; }
        public PlanoDeClassificacao PlanoDeClassificacao { get; private set; }
        public TemporalidadeEDestinacao TemporalidadeEDestinacao { get; private set; }

        #endregion

        #region Métodos de Entidade
        #endregion
        ...
        public override bool Equals(object o)
        {
            if (o.GetType().IsAssignableFrom(GetType()))
            {
                ...
                FieldInfo[] classeLocal = typeof(Classe).GetFields(visibilidadePermitida);
